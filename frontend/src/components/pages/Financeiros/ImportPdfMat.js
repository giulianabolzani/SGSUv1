...
function studentsPdf(students){
    ...
    const dados = students.map((student) => {
		return [
			{text: student.nome_st, fontSize: 9, margin: [0, 2, 0, 2]},
			{text: student.nome, fontSize: 9, margin: [0, 2, 0, 2]},
            {text: student.valor, fontSize: 9, margin: [0, 2, 0, 2]},
		]
	})
	...
	const details = [
		{
			table:{
				...
				body: [
					[
						{text: 'Aluno', style: 'tableHeader', fontSize: 10},
						{text: 'Material', style: 'tableHeader', fontSize: 10},
